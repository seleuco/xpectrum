...
void
UncompressZ80B (int tipo, int pc, Z80Regs * regs, void * fp)
{
  ...
  if ((tipo == Z80BL_V1UNCOMP) || (tipo == Z80BL_V2UNCOMP))
    {
      for (cont = pc; cont <= limit; cont++)
        Z80WriteMem_notiming(cont, *source++ );
    }
  else
    {
      last = 0;
      ...
      do
	{
	  ...
	  if ((last == 0xED) && (c == 0xED))
	  ...
	  else
	    {
	      ...
	      if ((last == 0xED) && (c != 0xED))
                Z80WriteMem_notiming(pc++,last);
	      /* If it's ED wait for the next or else to memory */
	      if (c != 0xED)
                Z80WriteMem_notiming(pc++,c);
	    }
	}
      while ((pc <= limit));
    }
...
void
UncompressZ80 (byte *dest, int tipo, int pc, Z80Regs * regs, void * fp)
{
  byte c, last, last2, dato;
  int cont, num, limit;
  byte *source=(byte *)fp;

//patch para mi ultima modificacion
  pc=0;

  limit = pc + ((tipo == Z80BL_V1UNCOMP || tipo == Z80BL_V1COMPRE) ? 0xbfff : 0x3fff);

  memset(dest,0,limit+1);
